...
const apiConfig = config.API.AUDITS;
...
export const getAuditsData = params => {
    return new Promise(async (resolve, reject) => {
        if(cachedData === null){
            // Logic can be applied to generate URL using params
            const url = formAuditsUrl(params);
            // console.log("Audits API calling...", url);
            try {
...
const formAuditsUrl = params => {
    let url = `${apiConfig.GET_AUDITS}`;
    let urlParams = {};
    if(params.dir){
        params.dir === 'next'
